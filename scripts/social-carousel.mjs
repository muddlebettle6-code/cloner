...
function getStoryboard(a, copy) {
  ...
  try {
    const prompt = `You are an explainer writer for a financial newsroom, writing for a GENERAL audience - anyone, not just finance people. Using ONLY this article's facts (invent nothing), turn it into a clear, engaging carousel that EXPLAINS the story so a complete beginner understands it. PLAIN LANGUAGE: write like you are explaining to a smart friend who knows nothing about finance; avoid jargon or define it in plain words (say "the price of oil" not "the front of the curve"; "money pulled out" not "redemptions"). Short, clear sentences. No hype, no investment advice, no em dashes. Use DIGITS with the dollar sign and spelled units (e.g. $72, $15.7 billion, 75 percent); never write a number as words, never use bn or M.\n\n`
      + `8 slides. INFORM on every slide: each slide makes ONE clear point a stranger understands on its own and actually DELIVERS the information - do not just tease it, and do NOT save the key facts for the end (front-load them, give the answer early). Slide 1 is a CLEAR HEADLINE anyone instantly gets (what happened and why it matters), not a cryptic riddle. Slides 2-7 explain the story step by step, each adding a useful, concrete point; keep it flowing and interesting but always clear and self-explanatory. Weave in 2-3 of the charts below where they help explain a point: set "chart" to the EXACT title and write "text" that plainly says what it shows. The LAST slide (role "cta") gives the one-line takeaway plus a save/share prompt. Each "text" is ONE clear line (<= ~16 words).\n\n`
